function [BestX, BestF, HisBestFit, VisitTable] = hybrid_AHA_mpa_single_NR4_Irr(MaxIt, nPop)
%==========================================================================
% Hybrid AHA-MPA Algorithm with Single Diode NR4 Objective
%
% This function implements a hybrid optimization algorithm combining:
% - Artificial Hummingbird Algorithm (AHA) features (flight modes, guided foraging)
% - Marine Predators Algorithm (MPA) features (elite-based movement, migration)
% - Adaptive control using visit tables and probability-based switching
%
% Inputs:
...
for i = 1:nPop
    PopFit(i) = objfunsdnr4irr(PopPos(i,:), 1);
end
...
for It = 1:MaxIt
    ...
    for i = 1:nPop
        ...
        newPopFit = objfunsdnr4irr(newPopPos, It);
    ...
    if mod(It, 2*nPop) == 0
        ...
        PopFit(MigrationIndex) = objfunsdnr4irr(PopPos(MigrationIndex,:), It);
        oppPopFit = objfunsdnr4irr(oppPopPos, It);
