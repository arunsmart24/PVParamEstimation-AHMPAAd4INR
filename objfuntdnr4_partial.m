function Fit=objfuntdnr4_partial(Xnew, dataset, C_Iter, MaxIteration)

%==========================================================================
% Adaptive Fourth-Order Newton-Raphson (Ad4INR) Objective Function for TDM
%
% This function calculates the RMSE fitness score for the Triple Diode Model
% (TDM) using a customized 4th-order iterative solver.
% TDM has 9 parameters.
%==========================================================================

% --- 1. Constants and Parameter Extraction (9 Parameters) ---
...
a1  = Xnew(3);  % Diode 1 ideality factor
a2  = Xnew(4);  % Diode 2 ideality factor
a3  = Xnew(5);  % Diode 3 ideality factor
Iph = Xnew(6);  % Photocurrent
Io1 = Xnew(7);  % Diode 1 saturation current
Io2 = Xnew(8);  % Diode 2 saturation current
Io3 = Xnew(9);  % Diode 3 saturation current

% Combined thermal voltage terms
kVT1 = a1 * VT; 
kVT2 = a2 * VT;
kVT3 = a3 * VT; % New TDM term
...
for j = 1:N 
    ...
    for iter = 1:MaxLocalIterations 
        ...
        % Exponentials for all three diodes
        exp_term1 = exp((V + I_calc * Rs) / kVT1);
        exp_term2 = exp((V + I_calc * Rs) / kVT2);
        exp_term3 = exp((V + I_calc * Rs) / kVT3); % New TDM term
        
        % 4.1. Implicit Function F(I) for TDM
        % F(I) = Iph - I_D1 - I_D2 - I_D3 - I_Rp - I = 0
        F_I = Iph - Io1 * (exp_term1 - 1) - Io2 * (exp_term2 - 1) - Io3 * (exp_term3 - 1) ...
              - ((V + I_calc * Rs) / Rp) - I_calc;
        
        % 4.2. First Derivative F'(I)
        % F'(I) = dF/dI
        FD_I = -(Io1 * dEdI1) * exp_term1 - (Io2 * dEdI2) * exp_term2 - (Io3 * dEdI3) * exp_term3 ...
               - (Rs / Rp) - 1;
        
        % 4.3. Fourth Derivative F''''(I)
        % F''''(I) = d^4F/dI^4
        FDDDD_I = -(Io1 * dEdI1^4) * exp_term1 - (Io2 * dEdI2^4) * exp_term2 - (Io3 * dEdI3^4) * exp_term3;
